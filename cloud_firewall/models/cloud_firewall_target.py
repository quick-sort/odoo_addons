...
class CloudFirewallTarget(models.Model):
    ...
    account_id = fields.Many2one(
        "cloud.account", string="云账号", required=True, ondelete="cascade", index=True
    )
    ...
    sync_log_ids = fields.One2many(
        "cloud.firewall.sync.log", "target_id", string="同步日志"
    )
    rules_ids = fields.One2many(
        "cloud.firewall.rule", "target_id", string="防火墙规则"
    )
    ...
    def _get_adapter(self):
        self.ensure_one()
        with self.account_id.work_on("cloud.account") as work:
