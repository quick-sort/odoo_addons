...
本模块组的三轴组合设计已完成使命，正在重新设计。原有设计文档（需求、
架构、决策记录、进度）已随 ``.kiro/`` 一并移除。

现有实现保留为**参考实现**：它验证过若干结论（去重身份归介质轴、载荷表
分离、``provider`` 必填默认 ``generic``），也在实践中暴露过问题（见
``infohub_arxiv/README.rst`` 的 ``_queue_channel()`` 说明）。重新设计时
可从中取材，但不必受其约束。
