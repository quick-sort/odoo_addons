...
patch(Chatter.prototype, {
  ...
  async ensureLLMThread() {
    ...
    // Find the default agent to seed the thread's provider/model/tools
    ...
    if (!agent) {
      throw new Error(
        "No AI agent found. Please configure an agent first."
      );
    }

    if (!agent.provider_id || !agent.model_id) {
      throw new Error(
        "The default agent has no provider or model configured. Please configure it first."
      );
    }

    // Create new thread linked to the agent - name will be auto-generated by backend
