...
class MailMessage(models.Model):
    ...
    def is_llm_agent_message(self):
        """Check if messages are LLM agent messages using the stored field."""
    ...
    def _message_fetch(self, domain, **kwargs):
        ...
        (user, agent, tool), not system notifications or other message types
    ...
    def set_user_vote(self, vote_value):
        ...
        if self.llm_role not in ("assistant", "tool"):
            raise UserError(
                _(
                    "Voting is only allowed on messages generated by the agent or a tool."
                )
            )
    ...
    def get_tool_calls(self):
        """Get tool calls from agent message body_json."""
        self.ensure_one()
