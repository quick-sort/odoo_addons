...
class DiscussChannel(models.Model):
    ...
    def _message_post_after_hook(self, message, msg_vals):
        ...
        odoobot_assistant = self._llm_discuss_odoobot_takeover_assistant(msg_vals)
        result = super()._message_post_after_hook(message, msg_vals)

        try:
            self.env["llm.assistant"]._llm_discuss_dispatch(
                self,
                message,
                msg_vals,
            )
        ...
        if odoobot_assistant:
            try:
                odoobot_assistant._llm_discuss_dispatch_odoobot(self, message)
            except Exception:  # noqa: BLE001 - never roll back an authorized user message
                _logger.exception(
    ...
    def _llm_discuss_user_can_use_assistant(self, assistant, user):
        """Check assistant availability for the message sender.
        ...
        if not user.has_group("base.group_user"):
            return bool(assistant.is_public)
        allowed = assistant.sudo()._get_allowed_assistants_for_user(user)
        return assistant in allowed

    def _llm_discuss_should_trigger(self, assistant, message, msg_vals):
        """Return whether ``assistant`` should reply to this channel message."""
        self.ensure_one()
    ...
    def _llm_discuss_odoobot_takeover_assistant(self, msg_vals):
        """Return the configured assistant for an eligible post-onboarding chat."""
        self.ensure_one()
        ...
        assistant = self.env["llm.assistant"]._get_odoobot_assistant()
        if not assistant or not self._llm_discuss_user_can_use_assistant(
            assistant,
            source_user,
        ):
            return self.env["llm.assistant"]
