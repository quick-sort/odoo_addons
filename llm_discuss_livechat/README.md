...
Companion module for [`llm_discuss`](../llm_discuss): lets an agent act as
...
- Adds **Live Chat Channels** to `llm.agent`.
- Keeps the current bot user's operator membership synchronized, including
  removing an old bot when the user is replaced, cleared, or the agent is
  deleted.
- Replies to visitor comments when the agent bot is the session's assigned
...
agent's dedicated low-privilege bot user rather than sudo. The queue marks
...
explicitly safe tools should be enabled for visitor-facing agents.
