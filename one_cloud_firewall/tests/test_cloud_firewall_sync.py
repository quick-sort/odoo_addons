...
class TestSyncConfig(TransactionCase):
    def test_get_singleton_creates_and_reuses(self):
        config_model = self.env["one.cloud.firewall.sync.config"]
    ...
    def _patch_fetch(self, ip):
        return mock.patch.object(
            type(self.env["one.cloud.firewall.sync.config"]),
            "_fetch_public_ip",
            return_value=ip,
        )

    def test_action_sync_now_success(self):
        account = self.env["one.cloud.account"].create(
            {"name": "DO 账号", "provider": "digitalocean", "do_api_token": "t"}
        )
        targets = self.env["one.cloud.firewall.target"].create(
            [
                {"name": "fw1", "account_id": account.id, "resource_id": "fw-1"},
                {"name": "fw2", "account_id": account.id, "resource_id": "fw-2"},
            ]
        )
        log_model = self.env["one.cloud.firewall.sync.log"]
    ...
    def test_action_sync_now_isolates_failures(self):
        ...
        with self._patch_fetch("203.0.113.7"), mock.patch.object(
            ...
            notification = targets.action_sync_now()
        bad_log = self.env["one.cloud.firewall.sync.log"].search(
            [("target_id.name", "=", "bad")]
        )
        ok_log = self.env["one.cloud.firewall.sync.log"].search(
            [("target_id.name", "=", "ok")]
        )
    ...
    def test_action_sync_now_empty_recordset_syncs_all(self):
        ...
        self.env["one.cloud.firewall.target"].search([]).unlink()
        account = self.env["one.cloud.account"].create(
            {"name": "DO 账号", "provider": "digitalocean", "do_api_token": "t"}
        )
        self.env["one.cloud.firewall.target"].create(
            {"name": "fw1", "account_id": account.id, "resource_id": "fw-1"}
        )
        with self._patch_fetch("203.0.113.7"), mock.patch.object(
            type(self.env["one.cloud.firewall.target"]), "_sync_target",
            return_value=("unchanged", "same"),
        ):
            self.env["one.cloud.firewall.target"].action_sync_now()
        self.assertEqual(
            len(self.env["one.cloud.firewall.sync.log"].search([])), 1
        )
    ...
    def test_cron_sync_all_runs_when_ip_unchanged(self):
        ...
        with mock.patch.object(
            ...
            type(self.env["one.cloud.firewall.target"]), "action_sync_now"
        ) as sync_mock:
            self.env["one.cloud.firewall.target"].cron_sync_all()
    ...
    def test_last_sync_computed(self):
        ...
        target = self.env["one.cloud.firewall.target"].create(
            {"name": "fw1", "account_id": account.id, "resource_id": "fw-1"}
        )
    ...
    def test_action_test_connection_no_provider(self):
        account = self.env["one.cloud.account"].new({"name": "x", "provider": False})
    ...
    def test_action_run_cron_triggers_sync(self):
        config = self.env["one.cloud.firewall.sync.config"]._get_singleton()
        with mock.patch.object(
            type(self.env["one.cloud.firewall.target"]), "cron_sync_all"
        ) as cron_mock:
    ...
    def test_gc_unchanged_logs_keeps_recent_and_other_states(self):
        ...
        Log = self.env["one.cloud.firewall.sync.log"]
        account = self.env["one.cloud.account"].create(
            {"name": "DO 账号", "provider": "digitalocean", "do_api_token": "t"}
        )
        target = self.env["one.cloud.firewall.target"].create(
    ...
    def _make_tencent_target(self):
        ...
        return self.env["one.cloud.firewall.target"].create(
    ...
    def test_rule_delete_last_does_not_clear_cloud(self):
        target = self._make_tencent_target()
        only = self.env["one.cloud.firewall.rule"].create(
            {"target_id": target.id, "protocol": "TCP", "port": "ALL",
             "cidr": "203.0.113.7/32", "remote": True}
        )
        fake_adapter = mock.MagicMock()
    ...
    def test_action_clear_rules_push_failure_keeps_local(self):
        target = self._make_tencent_target()
        self.env["one.cloud.firewall.rule"].create(
            {"target_id": target.id, "protocol": "TCP", "port": "ALL",
             "cidr": "203.0.113.7/32", "remote": True}
        )
        fake_adapter = mock.MagicMock()
    ...
    def test_action_push_rules_replaces_old_ip(self):
        ...
        rules = self.env["one.cloud.firewall.rule"]
        rules.create(
            {"target_id": target.id, "protocol": "TCP", "port": "ALL",
