...
class LLMStore(models.Model):
    _name = "llm.store"
    # ``collection.base`` makes this model a component collection, so store
    # adapters can be registered against it (see llm_store/components/);
    # ``llm.service.dispatch.mixin`` resolves them from ``service``.
    ...
    service = fields.Selection(
        # Empty here on purpose: the base addon ships no store. Each store addon
        # adds its key with ``selection_add`` plus an ``ondelete`` policy (see
        # llm_pgvector). A static list is what makes the value validated on
        # write and the label translatable.
        selection=[],
        required=True,
        tracking=True,
    )
