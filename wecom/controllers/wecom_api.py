...
class WecomApi(Controller):

    @route('/<string:filename>.txt', type='http', auth="public", methods=['GET'])
    def wecom_verify_jsdk_file_content(self, filename=None, **kw):
        """
        读取企微应用的JS-SDK域名校验文件内容，供企业微信核对域名归属。
        :return: wecom_js_sdk_file 的文件内容
        """
        app = request.env['wecom.app'].sudo().search(
            [('wecom_js_sdk_file_name', '=', f'{filename}.txt')], limit=1)
        if not app or not app.wecom_js_sdk_file:
            raise NotFound()
