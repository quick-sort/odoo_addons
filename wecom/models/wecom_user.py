...
class WecomUser(models.Model):
    ...
    wecom_id = fields.Char(string='企业微信UserId', required=True, index=True)
    name = fields.Char(required=True)
    department_ids = fields.Many2many('wecom.department', string='所属部门')
    main_department_id = fields.Many2one('wecom.department', string='主部门')
    ...
    _company_wecom_id_uniq = models.Constraint(
        ...
        '同一公司下企业微信成员UserId不能重复。')

    @api.model
    def sync_from_app(self, app, department_id=1, fetch_child=True):
        """
        通过指定的企微应用拉取成员数据，更新本地缓存。
        :return: 该公司下最新的全部 wecom.user 记录
        """
        app.ensure_one()
        data = app.get_users(department_id=department_id, fetch_child=fetch_child) or []
        company_id = app.company_id.id
        department_model = self.env['wecom.department']
        department_dict = {d.wecom_id: d.id for d in department_model.search([('company_id', '=', company_id)])}
        existing = {u.wecom_id: u for u in self.search([('company_id', '=', company_id)])}
        new_vals = []
        for item in data:
            job_number = next((attr.get('value') for attr in item.get('extattr', {}).get('attrs', [])
                                if attr.get('name') == '工号'), None)
            department_wecom_ids = item.get('department') or []
            department_ids = [department_dict[d] for d in department_wecom_ids if d in department_dict]
            main_department_wecom_id = item.get('main_department')
            vals = {
                'company_id': company_id,
                'wecom_id': item['userid'],
                'name': item['name'],
                'position': item.get('position', ''),
                'job_number': job_number,
                'department_ids': [(6, 0, department_ids)],
                'main_department_id': department_dict.get(main_department_wecom_id, False),
